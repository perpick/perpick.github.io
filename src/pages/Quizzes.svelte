...
<div class="relative pt-1 m-4">
  <div class="flex mb-2 items-center justify-between">
    <div>
      ...
      <span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-green-500 bg-pg-200">
        이게 향수 추천과 관련 있는지 궁금해 하는 중
      </span>
      {:else if Math.ceil(($pagination.page / ($pagination.max + 1)) *100) < 66}
      <span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-purple-500 bg-pb-200">
        이제 그만할까 생각하는 중
      </span>
      {:else}
      <span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-pink-500 bg-pp-200">
        거의 다 왔어! 힘을 내라구!
      </span>
      {/if}
    </div>
